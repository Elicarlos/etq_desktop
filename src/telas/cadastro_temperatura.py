...
class CadastroTemperatura(tk.Frame):
    tipos_carne = []
    ...
    def criar_widgets(self):
        ...
        self.temperatura_entry = ttk.Entry(self)
        self.temperatura_entry.grid(row=1, column=1, padx=10, pady=5, sticky="ew")
       
        salvar_produto = ttk.Button(self, text="Salvar", command=self.salvar_temperatura)
    ...
    def preencher_campos(self):
        if self.dados_produto:
            ...
            temperatura = self.dados_produto            
            temperatura = temperatura.temperatura       
            self.temperatura_entry.insert(0, temperatura)           
    ...
    def salvar_temperatura(self):
        ...
        if self.dados_produto:
            try:
                self.controller.atualizar_temperatura(self.dados_produto.id, dados_produto)
                messagebox.showinfo('Sucesso', 'Temperatura Atualizada com sucesso!')
                self.fechar_popup()
            
            except Exception as e:
                messagebox.showerror('Erro', f'Erro ao salvar o temperatura: {e}')
        
        else:
            try:            
                self.controller.salvar_temperatura(dados_produto)
                messagebox.showinfo("Sucesso", "Temperatura cadastrado com sucesso!")          
